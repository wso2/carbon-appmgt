...
public class WSO2APIPublisher implements APIPublisher {
    ...
    public boolean publishToStore(WebApp api,APIStore store) throws AppManagementException {
        ...
        if (store.getEndpoint() == null || store.getUsername() == null || store.getPassword() == null) {
            String msg = "External APIStore endpoint URL or credentials are not defined.Cannot proceed with publishing WebApp to the APIStore - "+store.getDisplayName();
    ...
    private boolean deleteWSO2Store(APIIdentifier apiId, String externalPublisher, String storeEndpoint, HttpContext httpContext) throws
                                                                                                                                  AppManagementException {
        boolean deleted=false;
        HttpClient httpclient = new DefaultHttpClient();
        if(storeEndpoint.contains("/store")){
            storeEndpoint=storeEndpoint.split("store")[0]+"publisher"+ AppMConstants.APISTORE_DELETE_URL;
        }
        else if(!generateEndpoint(storeEndpoint)){
            storeEndpoint=storeEndpoint+ AppMConstants.APISTORE_DELETE_URL;
        }
        HttpPost httppost = new HttpPost(storeEndpoint);

        List<NameValuePair> paramVals = new ArrayList<NameValuePair>();
        paramVals.add(new BasicNameValuePair(AppMConstants.API_ACTION, AppMConstants.API_REMOVE_ACTION));
        ...
        try {
            ...
            if (!isError) {  //If WebApp deletion success
            deleted=true;

            }
        } catch (UnsupportedEncodingException e) {
            throw new AppManagementException("Error while deleting the WebApp : "+ apiId.getApiName() +" from the external WSO2 APIStore : "+storeEndpoint + e);
     ...
     */
    private boolean authenticateAPIM(APIStore store,HttpContext httpContext) throws
                                                                             AppManagementException {
        try {
            ...
            storeEndpoint=store.getEndpoint().split("store")[0]+"publisher"+ AppMConstants.APISTORE_LOGIN_URL;
            }
            ...
            if (isError) {
                throw new AppManagementException(" Authentication with external APIStore : "+store.getDisplayName()+ "   failed: HTTP error code : " +
                          response.getStatusLine().getStatusCode()+".Failed WebApp publishing to APIStore: "+store.getDisplayName());
            ...
            throw new AppManagementException("Authentication with external APIStore : "+store.getDisplayName() +" failed.", e);
     ...
     * @param httpContext  HTTPContext
     */
    private boolean logoutFromExternalStore(APIStore store,HttpContext httpContext) throws
                                                                                    AppManagementException {
    	try {
            // create a post request to addAPI.
            HttpClient httpclient = new DefaultHttpClient();
            ...
            boolean isError=Boolean.parseBoolean(responseString.split(",")[0].split(":")[1].split("}")[0].trim());
            if (isError) {
                throw new AppManagementException(" Log out from external APIStore : "+store.getDisplayName()+ " failed: HTTP error code : " +
    ...
    private boolean addAPIToStore(WebApp api,String storeEndpoint,String externalPublisher, HttpContext httpContext) throws
                                                                                                                     AppManagementException {
        boolean added=false;
        HttpClient httpclient = new DefaultHttpClient();
        if(storeEndpoint.contains("/store")){
            storeEndpoint=storeEndpoint.split("store")[0]+"publisher"+ AppMConstants.APISTORE_ADD_URL;
        }
        else if(!generateEndpoint(storeEndpoint)){
            storeEndpoint=storeEndpoint+ AppMConstants.APISTORE_ADD_URL;
        }
        HttpPost httppost = new HttpPost(storeEndpoint);

        // Request parameters and other properties.
        List<NameValuePair> params =getParamsList(api, externalPublisher, AppMConstants.API_ADD_ACTION);
        ...
        try {
            ...
            String responseString = EntityUtils.toString(entity, "UTF-8");
            boolean isError=Boolean.parseBoolean(responseString.split(",")[0].split(":")[1].split("}")[0].trim());
            if (!isError) { //If WebApp creation success
            added=true;
            }
        } catch (UnsupportedEncodingException e) {
            throw new AppManagementException("Error while adding the WebApp:"+ api.getId().getApiName()+" to the external WSO2 APIStore:"+storeEndpoint + e);
    ...
    private boolean updateWSO2Store(WebApp api, String externalPublisher, String storeEndpoint, HttpContext httpContext) throws
                                                                                                                         AppManagementException {
        boolean updated=false;
        HttpClient httpclient = new DefaultHttpClient();
        if(storeEndpoint.contains("/store")){
        ...
        // Request parameters and other properties.
        List<NameValuePair> params = getParamsList(api, externalPublisher, AppMConstants.API_UPDATE_ACTION);
        ...
        try {
            ...
            String responseString = EntityUtils.toString(entity, "UTF-8");
            boolean isError=Boolean.parseBoolean(responseString.split(",")[0].split(":")[1].split("}")[0].trim());
            if (!isError) {   //If WebApp update success
                updated=true;

            }
        } catch (UnsupportedEncodingException e) {
            throw new AppManagementException("Error while updating the WebApp: "+ api.getId().getApiName()+" in the external WSO2 APIStore: "+storeEndpoint + e);
    ...
    private boolean publishAPIToStore(APIIdentifier apiId,String storeEndpoint,String externalPublisher, HttpContext httpContext) throws
                                                                                                                                  AppManagementException {
        boolean published=false;
        HttpClient httpclient = new DefaultHttpClient();
        if(storeEndpoint.contains("/store")){
            storeEndpoint=storeEndpoint.split("store")[0]+"publisher"+ AppMConstants.APISTORE_PUBLISH_URL;
        }
        else if(!generateEndpoint(storeEndpoint)){
            storeEndpoint=storeEndpoint+ AppMConstants.APISTORE_PUBLISH_URL;
        }
        HttpPost httppost = new HttpPost(storeEndpoint);

        List<NameValuePair> paramVals = new ArrayList<NameValuePair>();
        paramVals.add(new BasicNameValuePair(AppMConstants.API_ACTION, AppMConstants.API_CHANGE_STATUS_ACTION));
        paramVals.add(new BasicNameValuePair("name", apiId.getApiName()));
        paramVals.add(new BasicNameValuePair("provider", externalPublisher));
        paramVals.add(new BasicNameValuePair("version", apiId.getVersion()));
        paramVals.add(new BasicNameValuePair("status", AppMConstants.PUBLISHED));
        ...
        try {
            ...
            String responseString = EntityUtils.toString(entity, "UTF-8");
            boolean isError=Boolean.parseBoolean(responseString.split(",")[0].split(":")[1].split("}")[0].trim());
            if (!isError) {  //If WebApp publishing success
                published=true;

            }
        } catch (UnsupportedEncodingException e) {
            throw new AppManagementException("Error while publishing the WebApp: "+apiId.getApiName()+" to the external WSO2 APIStore : "+storeEndpoint + e);
    ...
    private List<NameValuePair> getParamsList(WebApp api,String externalPublisher, String action){
        // Request parameters and other properties.
        List<NameValuePair> params = new ArrayList<NameValuePair>();
        params.add(new BasicNameValuePair(AppMConstants.API_ACTION,action));
        ...
        //Setting current WebApp provider as the owner of the externally publishing WebApp
        ...
        params.add(new BasicNameValuePair("redirectURL", config.getFirstProperty(AppMConstants.EXTERNAL_API_STORES_STORE_URL)));
