...
public class APIProviderHostObject extends ScriptableObject {
            ...
            if (api != null) {
                ...
                if (oldStatus.equals(APIStatus.CREATED) && newStatus.equals(APIStatus.PUBLISHED)) {
                    ...
                    if (deprecateOldVersions) {
                        List<WebApp> apiList = apiProvider.getAPIsByProvider(provider);
                        APIVersionComparator versionComparator = new APIVersionComparator();
                        for (WebApp oldAPI : apiList) {
                            if (oldAPI.getId().getApiName().equals(name) && versionComparator.compare(oldAPI, api) < 0 && (oldAPI.getStatus().equals(APIStatus.PUBLISHED))) {
                ...
                handleException("Couldn't find an WebApp with the name-" + name + "version-" + version);
            }
        } catch (APIManagementException e) {
            handleException("Error while updating WebApp status", e);
            return false;
        }finally {
        	if (isTenantFlowStarted) {
        		PrivilegedCarbonContext.endTenantFlow();
    ...
    public static NativeArray jsFunction_getAPIsByProvider(Context cx, Scriptable thisObj,
        ...
        if (providerName != null) {
            ...
            try {
                ...
                List<WebApp> apiList = apiProvider.getAPIsByProvider(APIUtil.replaceEmailDomain(providerName));
                if (apiList != null) {
                    Iterator it = apiList.iterator();
                    int i = 0;
                    while (it.hasNext()) {
    ...
    public static void jsFunction_revokeAccessToken(Context cx, Scriptable thisObj,
        ...
        try {
            ...
            for (APIIdentifier apiIdentifier : apiIdentifierSet) {
                ...
                WebApp apiDefinition = apiProvider.getAPI(apiIdentifier);
    ...
    public static boolean jsFunction_updateExternalAPIStores(Context cx,Scriptable thisObj, Object[] args,
                ...
                APIProvider apiProvider = getAPIProvider(thisObj);
                        ...
                        APIIdentifier apiId = new APIIdentifier(provider, name, version);
                WebApp api = apiProvider.getAPI(apiId);
                //Getting selected external WebApp stores from UI and publish WebApp to them.
                        NativeArray externalAPIStores = (NativeArray) apiData.get("externalAPIStores", apiData);
