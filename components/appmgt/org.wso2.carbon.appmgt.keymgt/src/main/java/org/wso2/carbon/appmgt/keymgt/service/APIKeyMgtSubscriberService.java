...
public class APIKeyMgtSubscriberService extends AbstractAdmin {
    ...
    /**
     * Get the access token for a user per given WebApp. Users/developers can use this access token
     * to consume the WebApp by directly passing it as a bearer token as per the OAuth 2.0 specification.
     * @param userId User/Developer name
     * @param apiInfoDTO Information about the WebApp to which the Access token will be issued.
     *                   Provider name, WebApp name and the version should be provided to uniquely identify
     *                   an WebApp.
     ...
     * 2.0 bearer token to access any WebApp in the given application.
     *
     * @param userId User/Developer name
    ...
    public void revokeAccessTokenForApplication(Application application) throws APIManagementException, AxisFault {
        ...
        for (String key : keys) {
            ...
            for (SubscribedAPI api : apiSet) {
                ...
                WebApp apiDefinition = APIKeyMgtUtil.getAPI(api.getApiId());
