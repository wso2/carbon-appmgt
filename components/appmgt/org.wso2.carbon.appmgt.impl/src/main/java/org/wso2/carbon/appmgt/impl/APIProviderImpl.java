...
import org.wso2.carbon.appmgt.impl.publishers.WSO2APIPublisher;
...
class APIProviderImpl extends AbstractAPIManager implements APIProvider {
    ...
    public void addWebApp(WebApp api) throws APIManagementException {
        try {           
            ...
            throw new APIManagementException("Error in adding WebApp :"+api.getId().getApiName(),e);
        }
    }
    ...
    public void updateAPI(WebApp api) throws APIManagementException {
        ...
        if (oldApi.getStatus().equals(api.getStatus())) {
            try {
                ...
                handleException("Error while invalidating WebApp resource cache", axisFault);
            }

        } else {
            // We don't allow WebApp status updates via this method.
            // Use changeAPIStatus for that kind of updates.
            throw new APIManagementException("Invalid WebApp update operation involving WebApp status changes");
        }
    }



    private void updateApiArtifact(WebApp api, boolean updateMetadata,boolean updatePermissions) throws APIManagementException {
     ...
     */
    private void createUpdateAPIDefinition(WebApp api) throws APIManagementException {
    ...
    public void makeAPIKeysForwardCompatible(WebApp api) throws APIManagementException {
        ...
        for (String version : versions) {
            WebApp otherApi = getAPI(new APIIdentifier(provider, apiName, version));
    ...
    private void publishToGateway(WebApp api) throws APIManagementException {
    ...
    private void validateAndSetTransports(WebApp api) throws APIManagementException {
    ...
    private void removeFromGateway(WebApp api) throws APIManagementException {
        String tenantDomain = null;
        if (api.getId().getProviderName().contains("AT")) {
            String provider = api.getId().getProviderName().replace("-AT-", "@");
            tenantDomain = MultitenantUtils.getTenantDomain( provider);
    ...
    private boolean isAPIPublished(WebApp api) throws APIManagementException {
        try {
            String tenantDomain = null;
			if (api.getId().getProviderName().contains("AT")) {
				String provider = api.getId().getProviderName().replace("-AT-", "@");
    ...
    private APITemplateBuilder getAPITemplateBuilder(WebApp api){
        ...
        vtb.addHandler("org.wso2.carbon.appmgt.gateway.handlers.proxy.ReverseProxyHandler", Collections.EMPTY_MAP);
//
//        Map<String, String> properties = new HashMap<String, String>();
//        properties.put("id","A");
//        properties.put("policyKey","gov:" + APIConstants.API_TIER_LOCATION);
//        vtb.addHandler("org.wso2.carbon.appmgt.gateway.handlers.throttling.APIThrottleHandler",properties);
//
...
//        }
     ...
     *
     * @param api WebApp
     * @throws APIManagementException if failed to create WebApp
     */
    private void createAPI(WebApp api) throws APIManagementException {
        GenericArtifactManager artifactManager = APIUtil.getArtifactManager(registry,
                                                                            APIConstants.API_KEY);
    ...
    public void deleteAPI(APIIdentifier identifier) throws APIManagementException {
        ...
        try {
            ...
            if(subsCount > 0){
                handleException("Cannot remove the WebApp. Active Subscriptions Exist", null);
            }
            ...
            /*Remove WebApp Definition Resource - swagger*/
            ...
            WebApp api = new WebApp(identifier);
            // gatewayType check is required when WebApp Management is deployed on other servers to avoid synapse
     ...
     */
    @Override
    public void publishToExternalAPIStores(WebApp api, Set<APIStore> apiStoreSet)
            throws APIManagementException {
        ...
        if (apiStoreSet.size() > 0) {
            for (APIStore store : apiStoreSet) {
                if (APIConstants.WSO2_API_STORE_TYPE.equals(store.getType())) {
                    boolean published = new WSO2APIPublisher().publishToStore(api, store); //First trying to publish the WebApp to external APIStore
                    ...
                    log.warn("The configured external APIStore type is currently not supported.Hence ignoring publishing the WebApp to - " + store.getDisplayName());
                }
            }
     ...
     */
    @Override
    public boolean updateAPIsInExternalAPIStores(WebApp api, Set<APIStore> apiStoreSet)
            throws APIManagementException {
        ...
        for (APIStore apiStore: apiStoreSet) {
            ...
            for (APIStore store : publishedStores) {  //If selected external store in edit page is already saved in db
                if (store.equals(apiStore)) { //Check if there's a modification happened in config file external store definition
                    ...
                    publishedToStore=true; //Already the WebApp has published to external APIStore

                    //In this case,the WebApp is already added to external APIStore,thus we don't need to publish it again.
                    //We need to update the WebApp in external Store.
                    //Include to update WebApp in external APIStore
            ...
            if (!publishedToStore) {  //If the WebApp has not yet published to selected external APIStore
                notPublishedAPIStores.add(config.getExternalAPIStore(apiStore.getName()));
            }

        }
        //Publish WebApp to external APIStore which are not yet published
    ...
    /**
     * When enabled publishing to external APIStores support,updating the WebApp existing in external APIStores
     * @param api The WebApp which need to published
     * @param apiStoreSet The APIStores set to which need to publish WebApp
     * @throws org.wso2.carbon.apimgt.api.APIManagementException
     *          If failed to update subscription status
    ...
    private void updateAPIInExternalAPIStores(WebApp api, Set<APIStore> apiStoreSet)
            throws APIManagementException {
        if (apiStoreSet != null && apiStoreSet.size() > 0) {
            for (APIStore store : apiStoreSet) {
                if (APIConstants.WSO2_API_STORE_TYPE.equals(store.getType())) {//Check external APIStore type is wso2 or not
                    ...
                    log.warn("The configured external APIStore type is currently not supported.Hence ignoring updating the WebApp in - " + store.getDisplayName());
                }
            }
        }
