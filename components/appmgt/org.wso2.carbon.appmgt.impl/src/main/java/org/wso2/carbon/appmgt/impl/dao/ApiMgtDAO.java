...
public class ApiMgtDAO {
	...
	/**
	 * Get access token key for given userId and WebApp Identifier
	 * 
	 * @param userId
	 *            id of the user
	 ...
	 *            APIIdentifier
	 * @param keyType
	 *            Type of the key required
	 * @return Access token
	 * @throws APIManagementException
	 *             if failed to get Access token
	...
	public String getAccessKeyForAPI(String userId, String applicationName, APIInfoDTO identifier,
		...
		if (log.isDebugEnabled()) {
			log.debug("Searching for: " + identifier.getAPIIdentifier() + ", User: " +
			          tenantAwareUserId + ", ApplicationName: " + applicationName +
			          ", Tenant ID: " + tenantId);
		}
		...
		ResultSet rs = null;
		String sqlQuery =
		                  "SELECT " + "   SKM.ACCESS_TOKEN AS ACCESS_TOKEN " + "FROM "
		                          + "   AM_SUBSCRIPTION SP," + "   APPMGR_APP API,"
		                          + "   AM_SUBSCRIBER SB," + "   AM_APPLICATION APP, "
		                          + "   AM_SUBSCRIPTION_KEY_MAPPING SKM " + "WHERE "
		                          + "   SB.USER_ID=? " + "   AND SB.TENANT_ID=? "
		                          + "   AND API.API_PROVIDER=? " + "   AND API.API_NAME=?"
		                          + "   AND API.API_VERSION=?" + "   AND APP.NAME=? "
		                          + "   AND SKM.KEY_TYPE=? " + "   AND API.API_ID = SP.API_ID"
		...
		try {
			...
			while (rs.next()) {
				accessKey =
				            APIUtil.decryptToken(rs.getString(APIConstants.SUBSCRIPTION_FIELD_ACCESS_TOKEN));
			}
			...
			handleException("Error when decrypting access key for user : " + loginUserName +
	...
	public String getAccessKeyForApplication(String userId, String applicationName, String keyType)
		...
		// MultitenantUtils.getTenantAwareUsername(loginUserName);
		int tenantId = IdentityUtil.getTenantIdOFUser(loginUserName);
		...
		ResultSet rs = null;
		String sqlQuery =
		                  "SELECT " + "   IAT.ACCESS_TOKEN AS ACCESS_TOKEN " + "FROM " +
		                          "   AM_SUBSCRIBER SB," + "   AM_APPLICATION APP, " +
		                          "   AM_APPLICATION_KEY_MAPPING AKM," + accessTokenStoreTable +
		                          " IAT," + "   IDN_OAUTH_CONSUMER_APPS ICA " + "WHERE " +
		                          "   SB.USER_ID=? " + "   AND SB.TENANT_ID=? " +
	...
	public APIInfoDTO[] getSubscribedAPIsOfUser(String userId) throws APIManagementException,
		...
		ResultSet rs = null;
		String sqlQuery =
		                  "SELECT " + "   API.API_PROVIDER AS API_PROVIDER,"
		                          + "   API.API_NAME AS API_NAME,"
		                          + "   API.API_VERSION AS API_VERSION " + "FROM "
		                          + "   AM_SUBSCRIPTION SP, " + "   APPMGR_APP API,"
		                          + "   AM_SUBSCRIBER SB, " + "   AM_APPLICATION APP " + "WHERE "
		                          + "   SB.USER_ID = ? " + "   AND SB.TENANT_ID = ? "
		                          + "   AND SB.SUBSCRIBER_ID = APP.SUBSCRIBER_ID "
		                          + "   AND APP.APPLICATION_ID=SP.APPLICATION_ID "
	...
	public APIKeyInfoDTO[] getSubscribedUsersForAPI(APIInfoDTO apiInfoDTO)
		...
		// api_id store as "providerName_apiName_apiVersion" in AM_SUBSCRIPTION
		// table
		Connection conn = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		String sqlQuery =
		                  "SELECT " + "   SB.USER_ID, " + "   SB.TENANT_ID " + "FROM "
		                          + "   AM_SUBSCRIBER SB, " + "   AM_APPLICATION APP, "
		                          + "   AM_SUBSCRIPTION SP, " + "   APPMGR_APP API " + "WHERE "
		...
		try {
			...
			while (rs.next()) {
				...
				// int tenantId =
				// rs.getInt(APIConstants.SUBSCRIBER_FIELD_TENANT_ID);
	...
	public void changeAccessTokenStatus(String userId, APIInfoDTO apiInfoDTO, String statusEnum)
		...
		PreparedStatement ps = null;
		String sqlQuery =
		                  "UPDATE " + accessTokenStoreTable + " IAT , AM_SUBSCRIBER SB," +
		                          " AM_SUBSCRIPTION SP , AM_APPLICATION APP, APPMGR_APP API" +
		                          " SET IAT.TOKEN_STATE=?" + " WHERE SB.USER_ID=?" +
		                          " AND SB.TENANT_ID=?" + " AND API.API_PROVIDER=?" +
		                          " AND API.API_NAME=?" + " AND API.API_VERSION=?" +
	...
	public APIKeyValidationInfoDTO validateKey(String context, String version, String accessToken,
		...
		String applicationSqlQuery =
		                             "SELECT " + "   IAT.VALIDITY_PERIOD, " +
		                                     ...
		                                     "   APP.APPLICATION_TIER," + "   AKM.KEY_TYPE," +
		                                     "   API.API_NAME," + "   AKM.CONSUMER_KEY," +
		                                     "   API.API_PROVIDER" + " FROM " +
		...
		try {
			...
			if (rs.next()) {
				...
				issuedTime =
				             rs.getTimestamp(APIConstants.IDENTITY_OAUTH2_FIELD_TIME_CREATED,
				...
				if (requiredAuthenticationLevel.equals(APIConstants.AUTH_APPLICATION_LEVEL_TOKEN) ||
					...
					if (!(userType.equalsIgnoreCase(requiredAuthenticationLevel))) {
						keyValidationInfoDTO.setValidationStatus(APIConstants.KeyValidationStatus.API_AUTH_INCORRECT_ACCESS_TOKEN_TYPE);
						keyValidationInfoDTO.setAuthorized(false);
						return keyValidationInfoDTO;
					}
				}
				...
				if (APIConstants.TokenStatus.ACTIVE.equals(status)) {
					...
					if (validityPeriod != Long.MAX_VALUE &&
					    (currentTime - timestampSkew) > (issuedTime + validityPeriod)) {
						keyValidationInfoDTO.setValidationStatus(APIConstants.KeyValidationStatus.API_AUTH_ACCESS_TOKEN_EXPIRED);
						if (log.isDebugEnabled()) {
	...
	public void addSubscriber(Subscriber subscriber) throws APIManagementException {
		...
		try {
			...
			Application defaultApp =
			                         new Application(APIConstants.DEFAULT_APPLICATION_NAME,
			                                         subscriber);
		...
		try {
			...
			String query =
			               "SELECT" + " USER_ID, TENANT_ID, EMAIL_ADDRESS, DATE_SUBSCRIBED "
			                       + "FROM " + "AM_SUBSCRIBER" + " WHERE " + "SUBSCRIBER_ID=?";
			ps = conn.prepareStatement(query);
	...
	public int addSubscription(APIIdentifier identifier, String context, int applicationId,
	                           String status) throws APIManagementException {
		...
		try {
			conn = APIMgtDBUtil.getConnection();
			String getApiQuery =
			                     "SELECT API_ID FROM APPMGR_APP API WHERE API_PROVIDER = ? AND "
	...
	public void removeSubscription(APIIdentifier identifier, int applicationId)
		...
		try {
			...
			if (apiId == -1) {
				throw new APIManagementException("Unable to get the WebApp ID for: " + identifier);
			}
	...
	public Subscriber getSubscriber(String subscriberName) throws APIManagementException {
		...
		try {
			...
			if (result.next()) {
				subscriber =
				             new Subscriber(
				                            result.getString(APIConstants.SUBSCRIBER_FIELD_EMAIL_ADDRESS));
	...
	public Set<APIIdentifier> getAPIByConsumerKey(String accessToken) throws APIManagementException {
		...
		String getAPISql =
		                   "SELECT" + " API.API_PROVIDER," + " API.API_NAME," + " API.API_VERSION "
		                           + "FROM" + " AM_SUBSCRIPTION SUB,"
		                           + " AM_SUBSCRIPTION_KEY_MAPPING SKM, " + " APPMGR_APP API "
		...
		try {
			...
			handleException("Failed to get WebApp ID for token: " + accessToken, e);
		} catch (CryptoException e) {
			handleException("Failed to get WebApp ID for token: " + accessToken, e);
		} finally {
		...
		try {
			...
			String sqlQuery =
			                  "SELECT " + "   SUBS.SUBSCRIPTION_ID"
			                          ...
			                          + "   AM_SUBSCRIBER SUB," + "   AM_APPLICATION APP, "
			                          + "   AM_SUBSCRIPTION SUBS, " + "   APPMGR_APP API "
			                          + "WHERE " + "   SUB.USER_ID = ? "
			                          + "   AND SUB.TENANT_ID = ? "
			                          + "   AND SUB.SUBSCRIBER_ID=APP.SUBSCRIBER_ID "
			                          + "   AND APP.APPLICATION_ID=SUBS.APPLICATION_ID "
			                          + "   AND API.API_ID=SUBS.API_ID" + "   AND APP.NAME= ? ";
	...
	private Set<APIKey> getAPIKeysBySubscription(int subscriptionId) throws APIManagementException {
		...
		String getKeysSql =
		                    "SELECT " + " SKM.ACCESS_TOKEN AS ACCESS_TOKEN,"
		                            + " SKM.KEY_TYPE AS TOKEN_TYPE " + "FROM"
		                            + " AM_SUBSCRIPTION_KEY_MAPPING SKM " + "WHERE"
		                            + " SKM.SUBSCRIPTION_ID = ?";
		...
		try {
			...
			while (nestedRS.next()) {
				...
				String decryptedAccessToken =
				                              APIUtil.decryptToken(nestedRS.getString("ACCESS_TOKEN"));
				apiKey.setAccessToken(decryptedAccessToken);
				apiKey.setType(nestedRS.getString("TOKEN_TYPE"));
				apiKeys.add(apiKey);
			}
		} catch (SQLException e) {
			handleException("Failed to get WebApp keys for subscription: " + subscriptionId, e);
		} catch (CryptoException e) {
			handleException("Failed to get WebApp keys for subscription: " + subscriptionId, e);
		} finally {
			APIMgtDBUtil.closeAllConnections(ps, connection, result);
	...
	public APIKey getAccessTokenData(String accessToken) throws APIManagementException {
		...
		try {
			...
			while (getTokenRS.next()) {
				...
				String decryptedAccessToken =
				                              APIUtil.decryptToken(getTokenRS.getString("ACCESS_TOKEN")); // todo
																										  // -
																										  // check
																										  // redundant
																										  // decryption
	...
	private Set<APIKey> getApplicationKeys(String username, int applicationId)
		...
		try {
			APIKey productionKey =
			                       getProductionKeyOfApplication(username, applicationId,
			                                                     accessTokenStoreTable);
			...
			APIKey sandboxKey =
			                    getSandboxKeyOfApplication(username, applicationId,
			                                               accessTokenStoreTable);
			if (sandboxKey != null) {
	...
	private APIKey getProductionKeyOfApplication(String userName, int applicationId,
	                                             String accessTokenStoreTable) throws SQLException,
	                                                                          CryptoException,
	                                                                          APIManagementException {
		...
		String statement =
		                   " ICA.CONSUMER_KEY AS CONSUMER_KEY," +
		                           ...
		                           " AKM.KEY_TYPE AS TOKEN_TYPE " + "FROM" +
		                           " AM_APPLICATION_KEY_MAPPING AKM," + accessTokenStoreTable +
		                           " IAT," + " IDN_OAUTH_CONSUMER_APPS ICA " + "WHERE" +
		                           " AKM.APPLICATION_ID = ? AND" + " ICA.USERNAME = ? AND" +
		                           " IAT.USER_TYPE = ? AND" +
		...
		oracleSQL =
		            "SELECT ICA.CONSUMER_KEY AS CONSUMER_KEY," +
		                    ...
		                    " IAT.VALIDITY_PERIOD AS VALIDITY_PERIOD," +
		                    " AKM.KEY_TYPE AS TOKEN_TYPE " + " FROM" +
		                    " AM_APPLICATION_KEY_MAPPING AKM, " + accessTokenStoreTable + " IAT," +
		                    " IDN_OAUTH_CONSUMER_APPS ICA " + " WHERE" +
		                    " AKM.APPLICATION_ID = ? AND" + " ICA.USERNAME = ? AND" +
		                    " IAT.USER_TYPE = ? AND" + " ICA.CONSUMER_KEY = AKM.CONSUMER_KEY AND" +
		                    " IAT.CONSUMER_KEY = ICA.CONSUMER_KEY AND" +
		                    " IAT.TOKEN_SCOPE = 'PRODUCTION' AND" +
		                    " ICA.USERNAME = IAT.AUTHZ_USER AND" +
		                    " (IAT.TOKEN_STATE = 'ACTIVE' OR" + " IAT.TOKEN_STATE = 'EXPIRED' OR" +
		                    " IAT.TOKEN_STATE = 'REVOKED')" + " AND ROWNUM < 2 " +
		                    " ORDER BY IAT.TIME_CREATED DESC";
	...
	private APIKey getSandboxKeyOfApplication(String userName, int applicationId,
	                                          String accessTokenStoreTable) throws SQLException,
	                                                                       CryptoException,
	                                                                       APIManagementException {
		...
		oracleSQL =
		            "SELECT ICA.CONSUMER_KEY AS CONSUMER_KEY," +
		                    ...
		                    " IAT.TOKEN_SCOPE = 'SANDBOX' AND" +
		                    " ICA.USERNAME = IAT.AUTHZ_USER AND" +
		                    " (IAT.TOKEN_STATE = 'ACTIVE' OR" + " IAT.TOKEN_STATE = 'EXPIRED')" +
		                    " AND ROWNUM < 2 " + " ORDER BY IAT.TIME_CREATED DESC ";
	...
	public void updateTierPermissions(String tierName, String permissionType, String roles,
		...
		try {
			...
			String getTierPermissionQuery =
			                                "SELECT TIER_PERMISSIONS_ID FROM AM_TIER_PERMISSIONS WHERE TIER = ? AND TENANT_ID = ?";
	...
	public Set<TierPermissionDTO> getTierPermissions(int tenantId) throws APIManagementException {
		...
		try {
			conn = APIMgtDBUtil.getConnection();
			String getTierPermissionQuery =
			                                "SELECT TIER , PERMISSIONS_TYPE , ROLES  FROM AM_TIER_PERMISSIONS WHERE "
			                                        + "TENANT_ID = ?";
			ps = conn.prepareStatement(getTierPermissionQuery);
	...
	public TierPermissionDTO getTierPermission(String tierName, int tenantId)
		...
		try {
			conn = APIMgtDBUtil.getConnection();
			String getTierPermissionQuery =
			                                "SELECT PERMISSIONS_TYPE , ROLES  FROM AM_TIER_PERMISSIONS"
	...
	private String getKeysSql(String accessTokenStoreTable) {
		...
		return "SELECT " + " ICA.CONSUMER_KEY AS CONSUMER_KEY," +
		       " ICA.CONSUMER_SECRET AS CONSUMER_SECRET," + " IAT.ACCESS_TOKEN AS ACCESS_TOKEN," +
		       " AKM.KEY_TYPE AS TOKEN_TYPE " + "FROM" + " AM_APPLICATION_KEY_MAPPING AKM," +
		       tokenStoreTable + " IAT," + " IDN_OAUTH_CONSUMER_APPS ICA " + "WHERE" +
		       " AKM.APPLICATION_ID = ? AND" + " ICA.CONSUMER_KEY = AKM.CONSUMER_KEY AND" +
	...
	public Map<String, String> getAccessTokenData(int subscriptionId) throws APIManagementException {
		...
		if (APIUtil.checkAccessTokenPartitioningEnabled() &&
			...
			if (keyStoreTables != null) {
				for (String keyStoreTable : keyStoreTables) {
					apiKeys =
					          getAccessTokenData(subscriptionId,
	...
	private String getKeysSqlUsingSubscriptionId(String accessTokenStoreTable) {
		...
		return "SELECT " + " IAT.ACCESS_TOKEN AS ACCESS_TOKEN," +
		       " IAT.TOKEN_STATE AS TOKEN_STATE" + " FROM" + " AM_APPLICATION_KEY_MAPPING AKM," +
		       " AM_SUBSCRIPTION SM," + tokenStoreTable + " IAT," +
		       " IDN_OAUTH_CONSUMER_APPS ICA " + "WHERE" + " SM.SUBSCRIPTION_ID = ? AND" +
		       " SM.APPLICATION_ID= AKM.APPLICATION_ID AND" +
		       " ICA.CONSUMER_KEY = AKM.CONSUMER_KEY AND" + " ICA.CONSUMER_KEY = IAT.CONSUMER_KEY";
	}
		...
		try {
			...
			String sqlQuery =
			                  "SELECT " + "   SUBS.USER_ID AS USER_ID,"
			                          ...
			                          + "   AM_SUBSCRIBER  SUBS," + "   AM_APPLICATION  APP, "
			                          + "   AM_SUBSCRIPTION SUB, " + "   APPMGR_APP API "
		...
		try {
			...
			String sqlQuery =
			                  "SELECT DISTINCT "
			                          ...
			                          + "FROM AM_SUBSCRIBER SB, AM_SUBSCRIPTION SP,AM_APPLICATION APP,APPMGR_APP API"
			                          + " WHERE API.API_PROVIDER=? " + "AND API.API_NAME=? "
			                          + "AND API.API_VERSION=? "
			                          + "AND SP.APPLICATION_ID=APP.APPLICATION_ID"
			                          + " AND APP.SUBSCRIBER_ID=SB.SUBSCRIBER_ID "
			                          + " AND API.API_ID = SP.API_ID";
			...
			while (result.next()) {
				Subscriber subscriber =
				                        new Subscriber(
				                                       result.getString(APIConstants.SUBSCRIBER_FIELD_USER_ID));
	...
	public long getAPISubscriptionCountByAPI(APIIdentifier identifier)
		...
		String sqlQuery =
		                  "SELECT" + " COUNT(SUB.SUBSCRIPTION_ID) AS SUB_ID"
		                          + " FROM AM_SUBSCRIPTION SUB, APPMGR_APP API "
		                          + " WHERE API.API_PROVIDER=? " + " AND API.API_NAME=?"
		                          + " AND API.API_VERSION=?" + " AND API.API_ID=SUB.API_ID";
	...
	public void updateSubscription(APIIdentifier identifier, String subStatus, int applicationId)
		...
		try {
			...
			// This query to update the AM_SUBSCRIPTION table
			String sqlQuery =
			                  "UPDATE AM_SUBSCRIPTION SET SUB_STATUS = ? WHERE API_ID = ? AND APPLICATION_ID = ?";
	...
	public void updateAccessAllowDomains(String accessToken, String[] accessAllowDomains)
		...
		String sqlDeleteAccessAllowDomains =
		                                     "DELETE " + " FROM AM_APP_KEY_DOMAIN_MAPPING "
		                                             + " WHERE CONSUMER_KEY=?";

		String sqlAddAccessAllowDomains =
		                                  "INSERT"
	 ...
	 * @param keyType
	 *            Type (scope) of the key
	 * @return accessToken
	 * @throws IdentityException
	 *             if failed to register accessToken
	 */
	public String registerAccessToken(String consumerKey, String applicationName, String userId,
		...
		String getSubscriptionId =
		                           "SELECT SUBS.SUBSCRIPTION_ID " + "FROM "
		                                   + "  AM_SUBSCRIPTION SUBS, " + "  AM_APPLICATION APP, "
		                                   + "  AM_SUBSCRIBER SUB, " + "  APPMGR_APP API "
		                                   + "WHERE " + "  SUB.USER_ID = ?"
		                                   + "  AND SUB.TENANT_ID = ?"
		                                   + "  AND APP.SUBSCRIBER_ID = SUB.SUBSCRIBER_ID"
		                                   + "  AND APP.NAME = ?" + "  AND API.API_PROVIDER = ?"
		                                   + "  AND API.API_NAME = ?" + "  AND API.API_VERSION = ?"
		                                   + "  AND APP.APPLICATION_ID = SUBS.APPLICATION_ID"
		                                   + "  AND API.API_ID = SUBS.API_ID";

		String addSubscriptionKeyMapping =
		                                   "INSERT "
		                                           + "INTO AM_SUBSCRIPTION_KEY_MAPPING (SUBSCRIPTION_ID, ACCESS_TOKEN, KEY_TYPE) "
		                                           + "VALUES (?,?,?)";

		// String apiId =
		// apiInfoDTO.getProviderId()+"_"+apiInfoDTO.getApiName()+"_"+apiInfoDTO.getVersion();
		Connection connection = null;
		PreparedStatement prepStmt = null;
		try {
			consumerKey = APIUtil.encryptToken(consumerKey);
			...
			// throw new
			// IdentityException("Error when storing the access code for consumer key : "
			// + consumerKey);
		} catch (CryptoException e) {
			log.error(e.getMessage(), e);
			if (connection != null) {
	...
	public String registerApplicationAccessToken(String consumerKey, String applicationName,
	                                             String userId, int tenantId, String keyType,
		...
		String sqlAddAccessToken =
		                           "INSERT" +
		                                   " INTO " +
		                                   accessTokenStoreTable +
		                                   "(ACCESS_TOKEN, CONSUMER_KEY, TOKEN_STATE, TOKEN_SCOPE, AUTHZ_USER, USER_TYPE, TIME_CREATED, VALIDITY_PERIOD) " +
		                                   " VALUES (?,?,?,?,?,?,?,?)";

		String getApplicationId =
		                          "SELECT APP.APPLICATION_ID " + "FROM " + "  AM_APPLICATION APP, "
		                                  + "  AM_SUBSCRIBER SUB " + "WHERE " + "  SUB.USER_ID = ?"
		                                  + "  AND SUB.TENANT_ID = ?" + "  AND APP.NAME = ?"
		                                  + "  AND APP.SUBSCRIBER_ID = SUB.SUBSCRIBER_ID";

		String addApplicationKeyMapping =
		                                  "INSERT "
		...
		if (validityTime != null && !"".equals(validityTime)) {
			// When validity time getting passed from Jaggery, it append .0 to
			// the actual value.
			// Hence final value passed to backend contains decimals (eg:
			// 3600.0)
		...
		try {
			...
			handleException("Error while generating the application access token for the application :" +
			                        applicationName, e);
			if (connection != null) {
				try {
					connection.rollback();
	...
	public boolean isSubscribed(APIIdentifier apiIdentifier, String userId)
		...
		ResultSet rs = null;
		String sqlQuery =
		                  "SELECT " + "   SUBS.TIER_ID ," + "   API.API_PROVIDER ,"
		                          + "   API.API_NAME ," + "   API.API_VERSION ,"
		                          + "   SUBS.LAST_ACCESSED ," + "   SUBS.APPLICATION_ID " + "FROM "
		                          + "   AM_SUBSCRIPTION SUBS," + "   AM_SUBSCRIBER SUB, "
		                          + "   AM_APPLICATION  APP, " + "   APPMGR_APP API " + "WHERE "
		                          + "   API.API_PROVIDER  = ?" + "   AND API.API_NAME = ?"
		                          + "   AND API.API_VERSION = ?" + "   AND SUB.USER_ID = ?"
		                          + "   AND SUB.TENANT_ID = ? "
		                          + "   AND APP.SUBSCRIBER_ID = SUB.SUBSCRIBER_ID"
		                          + "   AND API.API_ID = SUBS.API_ID";
		...
		try {
			...
			String sqlQuery =
			                  "SELECT " + "   SUBS.SUBSCRIPTION_ID AS SUBSCRIPTION_ID, "
			                          ...
			                          + "   AM_APPLICATION APP, " + "   AM_SUBSCRIBER SUB, "
			                          + "   APPMGR_APP API " + "WHERE "
			                          + "   SUBS.APPLICATION_ID = APP.APPLICATION_ID "
			                          + "   AND APP.SUBSCRIBER_ID = SUB.SUBSCRIBER_ID "
			                          + "   AND API.API_PROVIDER = ? "
			                          + "   AND API.API_ID = SUBS.API_ID " + "ORDER BY "
			                          + "   APP.NAME";
			...
			while (result.next()) {
				...
				APIIdentifier apiId =
				                      new APIIdentifier(result.getString("API_PROVIDER"),
				                                        result.getString("API_NAME"),
				                                        result.getString("API_VERSION"));
	 ...
	 * @return Subscriber
	 * @throws APIManagementException
	 *             if failed to get subscriber for given access token
	 */
	...
	public String[] addOAuthConsumer(String username, int tenantId, String appName,
		...
		try {
			return new String[] { APIUtil.decryptToken(consumerKey),
			                     APIUtil.decryptToken(consumerSecret) };
		} catch (CryptoException e) {
	...
	public void addRating(APIIdentifier apiIdentifier, int rating, String userId, Connection conn)
		...
		try {
			...
			// the APPMGR_APP_RATINGS table
			String sqlQuery =
			                  "SELECT " + "RATING FROM APPMGR_APP_RATINGS "
			...
			String sqlAddQuery;
			if (!userRatingExists) {
				// This query to update the APPMGR_APP_RATINGS table
				sqlAddQuery =
				              "INSERT " + "INTO APPMGR_APP_RATINGS (RATING,API_ID, SUBSCRIBER_ID)"
				                      + " VALUES (?,?,?)";

			} else {
				// This query to insert into the APPMGR_APP_RATINGS table
				sqlAddQuery =
				              "UPDATE " + "APPMGR_APP_RATINGS SET RATING=? "
				                      + "WHERE API_ID= ? AND SUBSCRIBER_ID=?";
			}
			// Adding data to the APPMGR_APP_RATINGS table
	...
	public void removeAPIRating(APIIdentifier apiIdentifier, String userId, Connection conn)
		...
		try {
			...
			// the APPMGR_APP_RATINGS table
			String sqlQuery =
			                  "SELECT " + "RATING_ID FROM APPMGR_APP_RATINGS "
			                          + " WHERE API_ID= ? AND SUBSCRIBER_ID=? ";
			...
			if (rateId != -1) {
				...
				// APPMGR_APP_RATINGS table
				sqlAddQuery = "DELETE " + "FROM APPMGR_APP_RATINGS" + " WHERE RATING_ID =? ";
				// Adding data to the APPMGR_APP_RATINGS table
				ps = conn.prepareStatement(sqlAddQuery);
				ps.setInt(1, rateId);
				ps.executeUpdate();
	...
	public void updateApplication(Application application) throws APIManagementException {
		...
		try {
			...
			// This query to update the AM_APPLICATION table
			String sqlQuery =
			                  "UPDATE " + "AM_APPLICATION" + " SET NAME = ? "
	...
	public void updateApplicationStatus(int applicationId, String status)
		...
		try {
			...
			String updateSqlQuery =
			                        "UPDATE " + " AM_APPLICATION" + " SET APPLICATION_STATUS = ? "
	...
	public String getApplicationStatus(String appName, String userId) throws APIManagementException {
		...
		try {
			conn = APIMgtDBUtil.getConnection();
			String sqlQuery =
			                  "SELECT   APPLICATION_STATUS FROM   AM_APPLICATION " + "WHERE "
			                          + "   APPLICATION_ID= ?";
	...
	public int getApplicationId(String appName, String username) throws APIManagementException {
		...
		"FROM " + "   AM_APPLICATION " + "WHERE " + "   SUBSCRIBER_ID  = ? AND  NAME= ?";
	...
	public Application[] getApplications(Subscriber subscriber) throws APIManagementException {
		...
		String sqlQuery =
		                  "SELECT " + "   APPLICATION_ID " + "   ,NAME" + "   ,APPLICATION_TIER"
		                          + "   ,SUBSCRIBER_ID  " + "   ,CALLBACK_URL  "
		                          + "   ,DESCRIPTION  " + "   ,APPLICATION_STATUS  " + "FROM "
		                          + "   AM_APPLICATION " + "WHERE " + "   SUBSCRIBER_ID  = ?";
		...
		try {
			...
			while (rs.next()) {
				...
				Set<APIKey> keys =
				                   getApplicationKeys(tenantAwareUserId,
	...
	public void deleteApplication(Application application) throws APIManagementException {
		...
		String deleteApplicationKeyQuery =
	...
	public void recordAPILifeCycleEvent(APIIdentifier identifier, APIStatus oldStatus,
		...
		String getAPIQuery =
		                     "SELECT " + "API.API_ID FROM APPMGR_APP API" + " WHERE "
		                             + "API.API_PROVIDER = ?" + " AND API.API_NAME = ?"
		                             + " AND API.API_VERSION = ?";

		String sqlQuery =
		                  "INSERT "
		                          + "INTO APPMGR_APP_LC_EVENT (API_ID, PREVIOUS_STATE, NEW_STATE, USER_ID, TENANT_ID, EVENT_DATE)"
		                          + " VALUES (?,?,?,?,?,?)";
	...
	public void makeKeysForwardCompatible(String provider, String apiName, String oldVersion,
		...
		PreparedStatement prepStmt = null;
		ResultSet rs = null;
		String getSubscriptionDataQuery =
		                                  "SELECT"
		...
		String addSubKeyMapping =
		                          "INSERT INTO"
		                                  + " AM_SUBSCRIPTION_KEY_MAPPING (SUBSCRIPTION_ID, ACCESS_TOKEN, KEY_TYPE)"
		                                  + " VALUES (?,?,?)";

		String getApplicationDataQuery =
		                                 "SELECT" + " SUB.SUBSCRIPTION_ID AS SUBSCRIPTION_ID,"
		                                         + " SUB.TIER_ID AS TIER_ID,"
		                                         + " APP.APPLICATION_ID AS APPLICATION_ID,"
		                                         + " API.CONTEXT AS CONTEXT " + "FROM"
		                                         + " AM_SUBSCRIPTION SUB," + " AM_APPLICATION APP,"
		                                         + " APPMGR_APP API " + "WHERE"
		                                         + " API.API_PROVIDER = ?"
		                                         + " AND API.API_NAME = ?"
		                                         + " AND API.API_VERSION = ?"
		...
		try {
			...
			while (rs.next()) {
				...
				info.accessToken = rs.getString("ACCESS_TOKEN"); // no
																 // decryption
																 // needed.
			...
			for (SubscriptionInfo info : subscriptionData) {
				if (!subscriptionIdMap.containsKey(info.subscriptionId)) {
					...
					int subscriptionId =
					                     addSubscription(apiId, context, info.applicationId,
					                                     APIConstants.SubscriptionStatus.UNBLOCKED);
					if (subscriptionId == -1) {
						String msg =
						             "Unable to add a new subscription for the WebApp: " + apiName +
						                     ":v" + newVersion;
						log.error(msg);
						throw new APIManagementException(msg);
					}
	...
	public void addWebApp(WebApp api) throws APIManagementException {
		...
		String query =
		               "INSERT INTO APPMGR_APP(API_PROVIDER, API_NAME, API_VERSION, CONTEXT) "
		                       + "VALUES (?,?,?,?)";
	...
	public void updateWorkflowStatus(WorkflowDTO workflowDTO) throws APIManagementException {
		...
		String query =
		               "UPDATE AM_WORKFLOWS SET WF_STATUS = ?, WF_STATUS_DESC = ?, WF_UPDATED_TIME = ? "
	 ...
	 * @throws APIManagementException
	 */
	public void addURLTemplates(int apiId, WebApp api, Connection connection)
	                                                                         throws APIManagementException {
		if (apiId == -1) {
		...
		String query =
		               "INSERT INTO APPMGR_APP_URL_MAPPING (API_ID,HTTP_METHOD,AUTH_SCHEME,URL_PATTERN,THROTTLING_TIER) VALUES (?,?,?,?,?)";
		try {
			// connection = APIMgtDBUtil.getConnection();
			prepStmt = connection.prepareStatement(query);
	 ...
	 * @throws APIManagementException
	 */
	public void updateURLTemplates(WebApp api, Connection connection) throws APIManagementException {
		...
		PreparedStatement prepStmt = null;
		String deleteOldMappingsQuery = "DELETE FROM APPMGR_APP_URL_MAPPING WHERE API_ID = ?";
		try {
	...
	public static ArrayList<URITemplate> getAllURITemplates(String apiContext, String version)
		...
		String query =
		               "SELECT AUM.HTTP_METHOD,AUTH_SCHEME,URL_PATTERN,THROTTLING_TIER FROM APPMGR_APP_URL_MAPPING AUM, APPMGR_APP API "
		                       + "WHERE API.CONTEXT= ? "
		                       + "AND API.API_VERSION = ? "
		                       + "AND AUM.API_ID = API.API_ID " + "ORDER BY " + "URL_MAPPING_ID";
		try {
			connection = APIMgtDBUtil.getConnection();
			prepStmt = connection.prepareStatement(query);
	...
	public void updateAPI(WebApp api) throws APIManagementException {
		...
		String query =
		               "UPDATE APPMGR_APPSET CONTEXT = ? WHERE API_PROVIDER = ? AND API_NAME = ? AND"
		                       + " API_VERSION = ? ";
		try {
			connection = APIMgtDBUtil.getConnection();
	...
	public void deleteAPI(APIIdentifier apiId) throws APIManagementException {
		...
		String deleteLCEventQuery = "DELETE FROM APPMGR_APP_LC_EVENT WHERE API_ID=? ";
		String deleteCommentQuery = "DELETE FROM APPMGR_APP_COMMENTS WHERE API_ID=? ";
		String deleteRatingsQuery = "DELETE FROM APPMGR_APP_RATINGS WHERE API_ID=? ";
		...
		String deleteAPIQuery =
		                        "DELETE FROM APPMGR_APPWHERE API_PROVIDER=? AND API_NAME=? AND API_VERSION=? ";
		...
		try {
			...
			prepStmt.setInt(1, id);
			prepStmt.execute();
			// Delete all external APIStore details associated with a given
			// WebApp
	...
	/**
	 * Get APIIdentifiers Associated with access token - access token associated
	 * with application
	 * which has multiple APIs. so this returns all APIs associated with a
	 * access token
	 * 
	 * @param accessToken
	 *            String access token
	 * @return APIIdentifier set for all API's associated with given access
	 *         token
	 * @throws APIManagementException
	 *             error in getting APIIdentifiers
	 */
	public Set<APIIdentifier> getAPIByAccessToken(String accessToken) throws APIManagementException {
		...
		ResultSet result = null;
		String getAPISql =
		                   "SELECT AMA.API_ID,API_NAME,API_PROVIDER,API_VERSION FROM " +
		                           "APPMGR_APPAMA," +
		                           accessTokenStoreTable +
		                           " ACT, AM_APPLICATION_KEY_MAPPING AKM, " +
		...
		try {
			...
			ResultSet nestedRS = nestedPS.executeQuery();
			while (nestedRS.next()) {
				apiList.add(new APIIdentifier(
				                              APIUtil.replaceEmailDomain(nestedRS.getString("API_PROVIDER")),
				                              nestedRS.getString("API_NAME"),
				                              nestedRS.getString("API_VERSION")));
			}
		} catch (SQLException e) {
	...
	public Application[] getApplicationsByTier(String tier) throws APIManagementException {
		...
		String sqlQuery =
		                  "SELECT DISTINCT AMS.APPLICATION_ID,NAME,SUBSCRIBER_ID FROM AM_SUBSCRIPTION AMS,AM_APPLICATION AMA "
		                          + "WHERE TIER_ID=? "
	...
	public static List<URLMapping> getURITemplatesPerAPIAsString(APIIdentifier identifier)
		...
		try {
			...
			String sqlQuery =
			                  "SELECT " + "URL_PATTERN" + ",HTTP_METHOD" + ",AUTH_SCHEME"
			                          + ",THROTTLING_TIER " + "FROM " + "APPMGR_APP_URL_MAPPING "
			                          + "WHERE " + "API_ID = ? " + "ORDER BY "
			                          + "URL_MAPPING_ID ASC ";
	...
	public static String getAuthorizedDomains(String accessToken) throws APIManagementException {
		...
		String authorizedDomains = "";
		String accessAllowDomainsSql =
		                               "SELECT a.AUTHZ_DOMAIN " +
		...
		try {
			...
			throw new APIManagementException(
			                                 "Error in retrieving access allowing domain list from table.",
			                                 e);
		} catch (CryptoException e) {
			throw new APIManagementException(
			                                 "Error in retrieving access allowing domain list from table.",
	...
	public int addComment(APIIdentifier identifier, String commentText, String user)
		...
		try {
			...
			/* This query to update the APPMGR_APP_COMMENTS table */
			String addCommentQuery =
			                         "INSERT "
			                                 + "INTO APPMGR_APP_COMMENTS (COMMENT_TEXT,COMMENTED_USER,DATE_COMMENTED,API_ID)"
			                                 + " VALUES (?,?,?,?)";

			/* Adding data to the APPMGR_APP_COMMENTS table */
	...
	public Comment[] getComments(APIIdentifier identifier) throws APIManagementException {
		...
		String sqlQuery =
		                  "SELECT " + "   APPMGR_APP_COMMENTS.COMMENT_TEXT AS COMMENT_TEXT,"
		                          + "   APPMGR_APP_COMMENTS.COMMENTED_USER AS COMMENTED_USER,"
		                          + "   APPMGR_APP_COMMENTS.DATE_COMMENTED AS DATE_COMMENTED "
		                          + "FROM " + "   APPMGR_APP_COMMENTS, " + "   APPMGR_APP API "
		                          + "WHERE " + "   API.API_PROVIDER = ? "
		                          + "   AND API.API_NAME = ? " + "   AND API.API_VERSION  = ? "
		                          + "   AND API.API_ID = APPMGR_APP_COMMENTS.API_ID";
		try {
			connection = APIMgtDBUtil.getConnection();
			prepStmt = connection.prepareStatement(sqlQuery);
			prepStmt.setString(1, APIUtil.replaceEmailDomainBack(identifier.getProviderName()));
	...
	public static boolean isContextExist(String context) {
		...
		String sql = "SELECT CONTEXT FROM APPMGR_APP" + " WHERE CONTEXT= ?";
		try {
			connection = APIMgtDBUtil.getConnection();
	...
	public static List<String> getAllAvailableContexts() {
		...
		String sql = "SELECT CONTEXT FROM APPMGR_APP";
		try {
			connection = APIMgtDBUtil.getConnection();
			prepStmt = connection.prepareStatement(sql);
			resultSet = prepStmt.executeQuery();
	 ...
	 *             if failed to add Application
	 */
	public void addExternalAPIStoresDetails(APIIdentifier apiIdentifier, Set<APIStore> apiStoreSet,
		...
		try {
			...
			log.error("Error while adding External APIStore details to the database for WebApp : ",
			          e);
	...
	public void updateExternalAPIStoresDetails(APIIdentifier apiIdentifier,
		...
		try {
			...
			// This query to add external APIStores to database table
			String sqlQuery =
			                  "UPDATE " + "AM_EXTERNAL_STORES" + " SET " + "   STORE_ENDPOINT = ? "
			                          + "   ,STORE_TYPE = ? " + "WHERE "
			                          + "   API_ID = ? AND STORE_ID=?";
			...
			log.error("Error while updating External APIStore details to the database for WebApp : ",
			          e);
	...
	public Set<APIStore> getExternalAPIStoresDetails(APIIdentifier apiIdentifier, Connection conn)
		...
		try {
			...
			// This query to add external APIStores to database table
			String sqlQuery =
			                  "SELECT " + "   ES.STORE_ID, " + "   ES.STORE_DISPLAY_NAME, "
			                          + "   ES.STORE_ENDPOINT, " + "   ES.STORE_TYPE " + "FROM "
			                          + "   AM_EXTERNAL_STORES ES " + "WHERE "
			                          + "   ES.API_ID = ? ";
			...
			handleException("Error while getting External APIStore details from the database for  the WebApp : " +
