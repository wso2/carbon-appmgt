		...
		$.ajax({
			...
			success : function(response) {
				...
				if (element) {
					...
					if(current_state=='In-Review' & isAsynch==false){
						var alert = showBanner("Your request to publish the application is awaiting administrator approval.",'info',true);

						var circleElement = $('#Published');
						circleElement.attr("fill","#FF5050");
						paper.add([    
						{
						    type: "rect",
						    x: -3,
