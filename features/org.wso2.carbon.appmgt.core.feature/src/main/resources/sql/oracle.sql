...
CREATE TABLE APM_ONE_TIME_DOWNLOAD (
   BINARY_FILE VARCHAR(500) NOT NULL,
   UUID VARCHAR(500) NOT NULL,
   IS_DOWNLOADED NUMBER(0) NOT NULL,
   CREATED_TIME TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
   PRIMARY KEY (ID)
)
/

CREATE SEQUENCE APM_ONE_TIME_DOWNLOAD_SEQUENCE START WITH 1 INCREMENT BY 1 NOCACHE
/

CREATE OR REPLACE TRIGGER APM_ONE_TIME_DOWNLOAD_TRIGGER
    BEFORE INSERT
    ON APM_ONE_TIME_DOWNLOAD
    REFERENCING NEW AS NEW
    FOR EACH ROW
    BEGIN
        SELECT APM_ONE_TIME_DOWNLOAD_SEQUENCE.nextval INTO :NEW.ID FROM dual;
    END;
/
