...
CREATE SEQUENCE APM_ONE_TIME_DOWNLOAD_LINK_SEQUENCE START WITH 1 INCREMENT BY 1 NOCACHE
/

CREATE OR REPLACE TRIGGER APM_ONE_TIME_DOWNLOAD_LINK_TRIGGER
    BEFORE INSERT
    ON APM_ONE_TIME_DOWNLOAD_LINK
    REFERENCING NEW AS NEW
    FOR EACH ROW
    BEGIN
        SELECT APM_ONE_TIME_DOWNLOAD_LINK_SEQUENCE.nextval INTO :NEW.ID FROM dual;
    END;
/
